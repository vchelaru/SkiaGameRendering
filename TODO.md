...
| KNI             | 4.2.9001 (stock) | WebGL2 | Web | samples/Sample.Kni.WebGL/ | Production candidate | Option D implemented; Chrome/Edge/Firefox hardware acceptance measurements remain release gates. Pinned below latest (4.3.9001 crashes on startup — issue #14). |
...
[Issue #2](https://github.com/vchelaru/SkiaMonoGameRendering/issues/2) (closed) tracked the KNI upstream dependency: kniEngine/kni#2669 shipped in KNI v4.3.9001, so the KNI source patch/fork is gone (moved to stock NuGet). The one piece #2669 didn't cover — a public accessor for the current WebGL rendering context — is bridged with reflection (`WebGlCanvasUpload.cs`); pitching KNI a follow-up PR for it is tracked in [issue #13](https://github.com/vchelaru/SkiaMonoGameRendering/issues/13). `eng/Versions.props` pins `KniVersion` to 4.2.9001, not 4.3.9001, because 4.3.9001's BlazorGL platform crashes the WASM runtime on startup — reproduced independent of anything in this repo (gutted `WebGlCanvasUpload.cs` to inert stubs, still crashed; stock 4.2.9001 with the same stub doesn't). Tracked in [issue #14](https://github.com/vchelaru/SkiaMonoGameRendering/issues/14).
