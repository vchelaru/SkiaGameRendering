...
5. `src/SkiaGameRendering.Core.D3D12/` landed — engine-agnostic D3D12/Skia interop (Skia's Ganesh D3D12 backend, per issue #24's now-accepted deprecation risk), same "no host engine wired up yet" shape `Core.VK` started with. The MonoGame `WindowsDX12` glue project is next, but is blocked on `MonoGame/MonoGame#9536` ("Exposing Native GPU Handles") merging with a locked API shape — see `SkiaGameRendering-Notes.md` section 9 and issue #67 for why MG 3.8.5's native backend has no reachable device today.
