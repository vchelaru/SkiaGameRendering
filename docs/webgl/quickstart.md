...
The library is a Razor class library, so `skia-game-webgl.js` is delivered as a static web asset. Consumers do not copy JavaScript manually.

## Application contract

Render `<SkiaGameWebGlHost @ref="host" />`, await `host.Ready`, then construct `SkiaWebGlBackend` explicitly and initialize `SkiaRenderer`. Construct one `SkiaRenderTarget2D` per Skia surface you need. All graphics calls must stay on the browser graphics thread.
