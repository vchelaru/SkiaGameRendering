...
- **MG WindowsDX internal API shape.** Reflecting into SharpDX-wrapped internals can change between MG versions. Same fragility as current SDL reflection, different target. **This applies to MG 3.8.4 WindowsDX only — see the 3.8.5 note below.**
...
- ~~Does MG 3.8.5 expose `VkDevice` / `VkQueue` publicly, or is reflection still required?~~ **Answered, and it's worse than "reflection required."** MG 3.8.5's `DirectX` platform target (`MonoGame.Framework.Native.csproj`, not the old `MonoGame.Framework.WindowsDX.csproj`) is backed by a single native C/C++ library (`native/`). Confirmed by reading `MonoGame.Framework/Platform/Native/GraphicsDevice.Native.cs` at tag `v3.8.5`: `GraphicsDevice` holds `internal unsafe MGG_GraphicsDevice* Handle;` — an opaque pointer into that native library, not a `SharpDX`/`Vortice` `ID3D11Device`/`ID3D12Device` COM object. There is no managed device object left to reflect into, for D3D11 or D3D12 alike. Reaching a real `ID3D11Device`/`VkDevice` would mean going through MG's native interop layer (`MGG.*` P/Invoke surface) instead of C# reflection — a materially bigger undertaking than the reflection glue that ported WindowsDX in section 4. This means **D3D11-on-3.8.5 is not "the same as 3.8.4 WindowsDX, just unported"** — it has its own blocker, independent of the D3D12/Ganesh-deprecation risk in issue #24. Filed as [issue #67](https://github.com/vchelaru/SkiaGameRendering/issues/67).
- **Vulkan on MG 3.8.5 has the identical blocker, confirmed.** The same native library builds the Vulkan backend (`native/monogame/vulkan/MGG_Vulkan.cpp`); its `MGG_GraphicsDevice` struct holds the real `VkDevice`/`VkQueue` (`vulkan/MGG_Vulkan.cpp:249-253`), but the public C API (`native/monogame/include/api_MGG.h`) exposes no getter for them — only opaque draw/state calls (`Draw`, `SetTexture`, `Clear`, etc.). There is no exported path back to a `VkDevice` any more than there is to `ID3D11Device`. Same fix scope as D3D11/D3D12: it needs new exports added to MG's native API, not a client-side workaround.
